...
def apply_building_logic(features):
    ...
    if sector == "Residential" and timber_joists:
        features["Joisted Floors Material"] = "Timber Joists (Domestic)"
    elif sector == "Non-residential" and timber_joists:
        features["Joisted Floors Material"] = "Timber Joists (Office)"
...
def random_choice_conflicting_features(features, input_text):
    ...
    has_piles = features.get("Piles Material") is not None
    if not has_piles:
    ...
    if "raft" in input_text_lower:
        ...
        features["Raft Foundation Material"] = None
    elif features.get("Raft Foundation Material") and (
